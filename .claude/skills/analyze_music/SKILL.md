...
6. Infer harmonic context, preferably as chord symbols or roman numerals.
   ...
   - When a chromatic or otherwise dissonant passing/neighbor tone sits near a measure whose main notes could equally well be explained by more than one closely-related chord (e.g. ii vs. IV, both plausible predominants), don't stop checking once one candidate merely fits — compare which candidate that passing tone clashes against more gently. A half-step or other dissonance landing on a chord's root is far more exposed than the identical clash landing on that chord's 3rd or 5th, since the root is the chord's most structurally prominent tone. When candidates are otherwise equally well-supported, prefer whichever one lets the passing tone brush a less exposed chord tone rather than its root.

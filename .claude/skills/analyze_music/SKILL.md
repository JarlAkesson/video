...
2. Determine the key, and check for a mis-notated anacrusis. **Settle both before harmonizing** — these two decisions govern every chord that follows, and burying them inside the harmonic reasoning is how they go wrong unnoticed.
   - Never take the key from the key signature alone: it is ambiguous between relative major and minor, and a modulation between relatives does not change it. Decide from where the melody actually rests and resolves — above all the true final note — plus whether a raised leading tone appears and resolves up by a verified half step, and whether the opening outlines the candidate tonic triad. That last test falsifies rather than corroborates: if measure 1 would come out as III or vi, it is outlining another key's tonic, so the piece begins in that key and modulates. Test both ends before settling.
...
4. Infer harmonic context, preferably as chord symbols or roman numerals.
   ...
   **Dominant function**
   ...
   - A raised 4th between a predominant and V is often an augmented sixth, not an applied dominant. It+6 (b6-1-#4) keeps two tones of a preceding iv/ii and raises only one, where V/V shares none; its outer voices then converge on V's root by half step from both sides. Prefer it where the raised note inflects a note the predominant already had, and don't rank it by root motion — a linear chord's identity is its converging voices, not its bass. Revoice the preceding iv/ii as a first inversion (iv6), putting b6 in the bass: that is already the It+6's own bass note, so the bass holds it as a common tone and then steps down a half step to V.
   ...
   - Where the tonic arrives twice in the closing span — opening the last phrase and again ending it — read the first as the submediant (VI/vi), leaving the final cadence the phrase's only one rather than a repeat. The tonic's root and 3rd are both submediant tones, so the melody nearly always allows it, and the note that doubled the tonic's root becomes VI's 3rd, clearing the root-doubling check. Don't reject it on root motion: the slack step up from V is what a deceptive cadence is for.
   ...
   **Predominant Function**
   ...
   - Don't oscillate between two predominants (IV-ii-IV-ii): it progresses nowhere and reads as awkward voice leading. Across a two-measure predominant span prefer ii-IV-V-I over ii-ii-V-I where IV fits the second measure as well as ii does — a single forward move, not swapping back and forth. Where IV fits worse, hold the one predominant across both measures.
   ...
   **Regarding Dissonance**
   ...
   - For simple/nursery-style tunes, avoid assigning a chord that is dissonant with the melody note sounding above it — prefer the diatonic triad whose chord tones actually contain the melody note over one that only fits via an added 6th/9th or other color tone. This matters most for dominant chords: a clashing or colored dominant undercuts the cadential effect it's supposed to create. If no clean fit spans the measure, split it at the melodic change and give each half its own consonant chord. Exception: in minor/modal folk-style pieces, don't let this rule block a real V at a cadential moment just because the melody holds the tonic's minor 3rd (or another characteristic modal scale degree) against the chord's raised leading tone — that particular clash is a recognized, idiomatic feature of this style, not a fault, and is worth accepting when it completes a stronger cadential/harmonic-rhythm pattern (e.g. a regular i-V-i alternation).
   - Before splitting a measure or reaching for a different chord to avoid a dissonant-looking melody note, check whether extending the candidate chord — most often adding a 7th to the dominant (V7) — turns that note into a genuine chord tone instead. A note that clashes against a plain triad may already be the chord's 7th. Test this on every chord, not just V: a ♭7 above the tonic makes it V7/IV, so if the next measure is IV the note is a chord tone and the passage tonicizes the subdominant. Never call an accidental a scanning error while an applied dominant that would explain it is untested, and label it from the measure the ♭7 actually sounds in. Once the main clash is resolved that way, a lingering color tone (e.g. a 9th) is often worth accepting rather than splitting, because it lets the whole measure sit under one clean, regularly-placed chord — which is usually a better trade than breaking the harmonic rhythm to avoid minor extension-tone dissonance.
   ...
   **General**
   - Apply the step-2 key test per section, not once per piece: a raised 7th resolving up a half step to a local tonic establishes that minor key there (in the relative major the same note would be a raised 5th, with no function). If the final cadence confirms a different tonic, that's a real key change, not a tonicized vi or III — label each section in its own key and identify what joins them. A pivot chord belonging to both (e.g. VI in the minor doubling as IV in the relative major) is one option, but a deceptive V-III can carry the modulation alone, since III is already the incoming tonic. Prefer whichever gives the more consistent harmonic rhythm; don't insert a pivot that forces a mid-measure split the neighbours don't have.
   ...
   - On that zoom-out, apply the V's-5th check above with extra force to the piece's actual final measure(s): rather than end on an unsupported tonic pedal, take the V-I reading even if the change lands on an unusual beat (e.g. beat 4).

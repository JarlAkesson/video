...
6. Infer harmonic context, preferably as chord symbols or roman numerals.
   ...
   **Predominant Function**
   - Once a ii chord is used, let it lead to V — never let ii resolve directly back to I, whether once or repeatedly (ii-I-ii-I oscillation is the same mistake, just repeated). Unlike IV, which can idiomatically resolve straight to I (plagal motion), ii exists to prepare the dominant; "ii-I" as a resting point is a red flag that V got skipped. ii-I is fine only when I is a brief passing chord on the way to V, not the resolution itself — e.g. ii-I-V-I. If a phrase oscillates this way, check whether V7 fits its cadentially significant measure (usually the 3rd or 4th): ii's root and 3rd are also V7's 5th and minor 7th, so V7 often fits the same melody notes while turning the surrounding I into that legitimate passing chord and completing a real authentic cadence instead. Even in an already-valid ii-I(passing)-V-I, prefer IV over ii if the melody notes center on ii's 3rd and 5th rather than its root: those same two notes are IV's root and 3rd, and IV's root motion into the following I (a descending 4th) is much stronger than ii's (a plain descending step).
...
7. Verify the draft harmony before emitting anything. This is a separate, mandatory pass over the finished chord-per-measure draft, not another item to weigh while composing it — citing a rule while drafting is not the same as checking the draft against it.
   - For every measure, determine whether its chord was freshly derived from that measure's own notes or simply carried over from the previous measure. Carried-over chords are exactly where mistakes hide, since nothing prompted a fresh look.
   - For each carried-over chord, compute (don't eyeball) whether every strong-beat melody note is either a genuine tone of that chord, or a suspension/passing tone that resolves by step into one on the very next note — use the Bash tool with music21 for this. A note that is neither is a hard failure: the chord is wrong for that measure and must be reconsidered, full stop, regardless of how plausible the surrounding phrase looked. Don't gate this on how harsh the clash sounds — a perfect 4th above a chord's root is a valid suspension despite being a harsh acoustic dissonance; the resolution is the test, not the interval size.
   - Run this for every measure, not only ones that already look uncertain — today's failures were all in measures that looked "obviously fine" until actually checked.
   - Only emit `music_analysis.json` once this pass is complete and clean.
8. Emit `music_analysis.json`.
...
- When promoting a decorative note to a real chord, apply the same ii/vi-over-IV check from step 6 above, and use it deliberately to spread different predominant colors across a repeated phrase pair rather than reusing the same one everywhere.

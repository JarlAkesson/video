...
1. Infer harmonic context, preferably as chord symbols or roman numerals.
   ...
   **Dominant function**
   ...
   - Never open a piece on V: the first measure has to establish the tonic, so read an ambiguous opening measure as I even when the dominant fits its notes just as well (scale degree 5 on the downbeat is I's 5th, not automatically V's root). Then harmonize any later repeat of that same opening figure as I too, for consistency.

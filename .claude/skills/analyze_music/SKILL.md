...
7. Verify the draft harmony before emitting anything. This is a separate, mandatory pass over the finished chord-per-measure draft, not another item to weigh while composing it — citing a rule while drafting is not the same as checking the draft against it.
   ...
   - For each carried-over chord, compute (don't eyeball) the interval from every strong-beat melody note to every tone of that chord — use the Bash tool with music21 for this. If a note is dissonant (e.g. a half step) against *all* of the chord's tones, the chord is wrong for that measure and must be reconsidered, full stop, regardless of how plausible the surrounding phrase looked.
